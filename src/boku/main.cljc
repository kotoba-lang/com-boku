(ns boku.main
  "Kotodama WASM entrypoint for the Boku clean-room actor (L4) — Clojure port.
  ...
  facts (`boku.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"boku.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "boku-compat" :tier tier :entities entities} 200])
